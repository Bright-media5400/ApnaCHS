app.controller('approveFlatsController', [
...
function ($scope, $rootScope, $window, $location, $http, $routeParams, $anchorScroll, $filter, DATA_URLS, appConstant, localStorageService, $sce, authService) {
    ...
    function getList() {
        ...
        var promise = $http.post(DATA_URLS.LIST_FLATS, filters);
    ...
    $scope.approve = function () {
        ...
        var promise = $http.post(DATA_URLS.APPROVE_FLAT, fdata);
        ...
        promise.then(function (data) {
            if (!data.status || data.status == 200) {
                ...
                toastr["success"]("Flat approved by you", "Success");
    ...
    $scope.reject = function (item, index) {
        var fdata = { id: $scope.selectedItem.id, note: $scope.note };
        var promise = $http.post(DATA_URLS.REJECT_FLAT, fdata);
        promise.then(function (data) {
            if (!data.status || data.status == 200) {
                $scope.recordList[$scope.selectedItem.index].isRejected = true;
                toastr["success"]("Flat rejected", "Success");
    ...
    $scope.approveAll = function () {
        ...
        var promise = $http.post(DATA_URLS.BULKAPPROVE_FLAT, fdata);
