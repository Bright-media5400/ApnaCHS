app.controller('approveFlatownersController', [
...
function ($scope, $rootScope, $window, $location, $http, $routeParams, $anchorScroll, $filter, DATA_URLS, appConstant, localStorageService, $sce, authService) {
    ...
    $scope.commentTableName = "FlatOwner";
    ...
    function getList() {
        ...
        var promise = $http.post(DATA_URLS.LIST_FLATOWNERS, filters);
    ...
    $scope.approve = function () {
        var soc = (JSON.parse(localStorageService.get('onLogSoc')));
        var fdata = { flatOwner: $scope.selectedItem.id, societyId: soc.id, note: $scope.note };
        ...
        promise.then(function (data) {
            if (!data.status || data.status == 200) {
                ...
                toastr["success"]("Flat owner approved by you", "Success");
    ...
    $scope.reject = function (item, index) {
        ...
        var promise = $http.post(DATA_URLS.REJECT_FLATOWNER, fdata);
        promise.then(function (data) {
            if (!data.status || data.status == 200) {
                $scope.recordList[$scope.selectedItem.index].isRejected = true;
                toastr["success"]("Flat owner rejected", "Success");
    ...
    $scope.approveAll = function () {
        ...
        var promise = $http.post(DATA_URLS.BULKAPPROVE_FLATOWNER, fdata);
