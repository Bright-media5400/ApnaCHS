'use strict';
app.factory('authService', ['$q', '$injector', 'localStorageService', 'appConstant', '$rootScope', 'DATA_URLS','toastr',
function ($q, $injector, localStorageService, appConstant, $rootScope, DATA_URLS,toastr) {
    ...
    var clearAuthData = function () {
        ...
        localStorageService.remove('onLogSoc');
    }
    ...
    var navigateUser = function(roles) {
        if(authentication.loginData.type == 1){
            logout();
            toastr["error"]('Login not allowed', "Error");            
            return false;
        }
        else{
            var r = JSON.parse(roles);
            if(contains.call(r, 3) || contains.call(r, 4)){
                authentication.loginData.role = "Admin";
            }
            else if(contains.call(r, 5) || contains.call(r, 6)){
                authentication.loginData.role = "Member";
            }
            else if(contains.call(r, 7)){
                authentication.loginData.role = "Tenant";
            }
            else if(contains.call(r, 8)){
                authentication.loginData.role = "MFamily";
            }
            else if(contains.call(r, 9)){
                authentication.loginData.role = "TFamily";
            }
            else{
                logout();
                toastr["error"]('Login not allowed', "Error");    
